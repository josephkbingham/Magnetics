...
| Core operating limits | Candidate material, $B_{max}$, hot $B_{sat}$, allowed $\Delta B$, saturation margin, core-loss curve or core-loss model, and whether the design is saturation-limited or loss-limited. For discrete-gapped ferrite inductors, hot $B_{sat}$ and loss curves dominate material selection more than raw $\mu_r$ once the gap reluctance dominates. [source: TI SLUP132 §5 Design Strategy] [source: Erickson & Maksimovic Ch. 10 §10.3.1] [source: Hurley & Wolfle Ch. 1 §1.3] |
| Core geometry candidates | Core family/shape, $A_e$, $A_w$, $V_e$, $l_e$, $MLT$, center-pole dimension for gap fringing, manufacturer $A_L$ data, and available gap or effective-permeability options. Discrete-gapped designs require available gap/$A_L$ options; distributed-gap powder designs require a selected $\mu_{eff}$ grade. [source: TI SLUP132 §5 Design Strategy] [source: Hurley & Wolfle Ch. 2 §2.3.2] [source: Hurley & Wolfle Ch. 3 §3.2] |
